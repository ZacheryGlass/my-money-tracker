...
UPDATE evm_audit_jobs
 ...
 WHERE EXISTS (
         SELECT 1
           FROM jsonb_array_elements(requested_chains) AS requested(value)
          WHERE trim(both '"' from requested.value::text) = '8453'
       )
    OR EXISTS (
         SELECT 1
           FROM jsonb_array_elements(discovered_chains) AS discovered(value)
          WHERE trim(both '"' from discovered.value::text) = '8453'
       );
DELETE FROM evm_audit_jobs
 WHERE requested_chains = '[]'::jsonb
    OR NOT EXISTS (
         SELECT 1
           FROM jsonb_array_elements(requested_chains) AS requested(value)
          WHERE trim(both '"' from requested.value::text) <> ''
       );
...
-- The retired seed also used the two shared OP Stack predeploy addresses for
-- Base and named them "OP Stack".  The current seed gives those same
-- addresses their OP Mainnet identity.  Repair only the exact stale builtin
-- names; a user-specific label (or a manually corrected builtin row) is left
-- untouched, and the address remains a valid bridge label for chain 10.
UPDATE eth_address_labels
   SET name = CASE address
                WHEN '0x4200000000000000000000000000000000000010'
                  THEN 'Optimism: L2 Standard Bridge'
                WHEN '0x4200000000000000000000000000000000000016'
                  THEN 'Optimism: L2 To L1 Message Passer'
              END,
       note = 'Cross-chain bridge on chain 10. Source: https://raw.githubusercontent.com/ethereum-optimism/superchain-registry/main/superchain/extra/addresses/addresses.json'
 WHERE user_id IS NULL
   AND source = 'builtin-bridge'
