...
test('ordinary native-credit feeds exclude Base while audit metadata remains available', () => {
...
test('the legacy native-credit feed runs only on its declaring chains', async (t) => {
  ...
  assert.deepEqual(stateSyncChains, [137, 100, 10], 'only declaring chains fetch the feed');
...
test('an empty successful native-credit scan advances to the scanned chain head', async (t) => {
  ...
  const { calls } = harness(t, {
    chainSet: '10',
    feedBehavior: { '10:statesync': empty },
  });

  await EthWalletService.syncWallet(7);

  const cursorUpdate = calls.cursors.find((c) => c.chainId === 10);
