...
Tables: `accounts`, `holdings`, `price_cache`, `ticker_snapshots`, `account_snapshots`, `users`, `user_identities`, `user_api_keys`, `app_settings`, `eth_wallets`, `eth_wallet_chains`, `eth_transfers`, `eth_ignored_tokens`, `eth_address_labels`, `eth_method_signatures`, `exchange_accounts`, `exchange_records`. Migrations in `backend/migrations/`.

## Environment Variables

Backend `.env`: `DATABASE_URL`, `ALLOWED_PRINCIPALS` (prod only), `MCP_API_KEY`, `SECRETS_ENCRYPTION_KEY` (enables DB-stored API keys), `CMC_PRO_API_KEY`, `CG_API_KEY`, `ETHERSCAN_API_KEY`, `PLAID_CLIENT_ID`/`PLAID_SECRET`/`PLAID_ENV` (env values are fallbacks below DB-stored keys), `PORT`, `NODE_ENV`, `RUN_SCHEDULED_JOBS`, `ETH_CHAINS` (comma-separated chain ids to sync; unset = registry defaults, `1` = mainnet only), `DEV_AUTH_USER_ID`/`DEV_AUTH_USERNAME` (dev identity)
...
- **Testing**: 376 backend (node:test + supertest, fake pg Pool via require.cache) and 60 frontend (vitest) — broad on scoping/secrets/admin/CSV import/multi-chain sync, thin on charts and analytics
