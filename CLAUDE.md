...
- **Builtin label pack** (migrations 035/036, `backend/scripts/generate-label-seed.js`): `eth_address_labels` carries `source` (`user` | `builtin` | `eth-labels`) and `confidence` (`high` | `low` | NULL on user rows). 036 seeds 5,129 addresses scraped from the eth-labels dataset (4,859 `exchange`, 270 `external` from its `dex` tag) as `user_id NULL, source='eth-labels', confidence='low'`; regenerate both artifacts (`backend/data/builtin-address-labels.json` + the migration) from the 21MB dump, which is NOT committed. Neither column votes: precedence is still user-shadows-builtin and own-beats-exchange. **Traps**: (1) 035's backfill is scoped to `source='user'`, not to `user_id IS NULL` — it runs before 036 on every boot, so the unscoped form would relabel the whole scraped pack as hand-verified; (2) the seed is `ON CONFLICT (address) WHERE user_id IS NULL DO NOTHING`, never DO UPDATE, so a re-boot cannot re-stamp a name or kind the user corrected, and 029's hand-verified rows (inserted first, `confidence='high'`) win every overlap; (3) `findAllForUser` hides pack rows from the Settings management list (filtered outside the DISTINCT ON, after shadowing resolves) — 5k scraped rows would bury the user's own labels; `findByAddress` and classification still see them.
